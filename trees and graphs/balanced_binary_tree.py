...
def is_balanced(tree_root) -> bool:
    ...
    if tree_root.left and tree_root.right:
        LEVELS.append(tree_root.left.value)
        LEVELS.append(tree_root.right.value)
        LEVEL += 1
    if tree_root.left:
        if tree_root.left.value not in LEVELS:
            LEVELS.append(tree_root.left.value)
            LEVEL += 1
        print(tree_root.value)
        is_balanced(tree_root.left)
    if tree_root.right:
        if tree_root.right.value not in LEVELS:
            LEVELS.append(tree_root.right.value)
            LEVEL += 1
        print(tree_root.value)
        is_balanced(tree_root.right)
